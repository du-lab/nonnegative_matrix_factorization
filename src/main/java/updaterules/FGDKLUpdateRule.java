...
public class FGDKLUpdateRule extends RegularizationUpdateRule
{
            ...
            for (int j = 0; j < numSamples; ++j) {
                ...
                for (int l = 0; l < numFeatures; ++l) {
                    ...
                    if (Double.isFinite(fraction)) {
                        a += x.get(l, j) * fraction * fraction;
                        dphi += wd.get(l, j) - x.get(l, j) * fraction;
                    }
                }
    ...
    private DoubleMatrix getNabla(@Nonnull DoubleMatrix x, @Nonnull DoubleMatrix w, @Nonnull DoubleMatrix h)
    {
        ...
        DoubleMatrix nabla = wt.mmul(x.div(w.mmul(h).max(EPS)))
                .div(h.mul(mu).add(lambda).addColumnVector(wt.rowSums()).max(EPS));
        nabla.muli(h);
        nabla.subi(h);
